...
import { CrawlerParams, runMinigtTask } from '../../../api/carDataCrawlerApi';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (productPageUrl || range) {
            ...
            for (const uc of urlChunks) {
                ...
                try {
                    runMinigtTask(crawlBody).catch(err => console.log(err));
                } catch (err: any) {
    ...
    const getRangeUrls = (range: string) => {
        ...
        if (Number.isNaN(left) || Number.isNaN(right)) {
            ...
            return new Array(right - left + 1)
                .fill(null)
                .map((u, index) => `https://minigt.tsm-models.com/index.php?action=product-detail&id=${left + index}`);
        }
    ...
    return (
        <div className='crawler-container'>
            <form onSubmit={handleSubmit}>
                ...
                <label>
                    <div>Catalog Url (currently disabled)</div>
                    <input
                        value={catalogUrl}
                        onChange={(e) => {setCatalogUrl(e.target.value); setProductPageUrl(''); setRange('');}}
                        placeholder="catalog page url"
                    />
                    <div>
                        For example
                        <ul>
                            <li>MINIGT: https://minigt.tsm-models.com/index.php?action=product-list&b_id=13&p=1</li>
                        </ul>
                    </div>
                </label>
                ...
                <label>
                    ...
                    <textarea
                        ...
                        onChange={(e) => { setProductPageUrl(e.target.value); setCatalogUrl(''); setRange('');}}
                        placeholder="page urls"
                    />
                    <div>
                        For example
                        <ul>
                            <li>https://minigt.tsm-models.com/index.php?action=product-detail&id=2070</li>
                        </ul>
                    </div>
                    <div>Use a Range</div>
                    <input
                        value={range}
                        onChange={(e) => { setRange(e.target.value); setProductPageUrl(''); setCatalogUrl('');}}
                        placeholder="1-10"
                    />
                    <div>
                        For example
                ...
                <div>
                    ...
                    <button
                        type="button"
                        onClick={() => { setCatalogUrl(""); setError(null); setResponse(null); }}
                    >
                        Reset
                    </button>
                </div>
